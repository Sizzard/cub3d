...
{
	if (ft_char_is_checkable(map[i][j]) == 0 && j > 0 && (int)ft_strlen(map[i
			- 1]) >= j && ft_char_is_ok(map[i - 1][j]) == 0
		&& (int)ft_strlen(map[i + 1]) >= j && ft_char_is_ok(map[i + 1][j]) == 0
		&& ft_char_is_ok(map[i][j + 1]) == 0 && ft_char_is_ok(map[i][j
			- 1]) == 0)
		return (0);
	else
		return (1);
...
{
	if (ft_char_is_checkable(map[i][j]) == 0 && (j == 0 || (int)ft_strlen(map[i
				- 1]) < j || ft_char_is_ok(map[i - 1][j]) == 1
			|| (int)ft_strlen(map[i - 1]) < j || ft_char_is_ok(map[i
				+ 1][j]) == 1 || ft_char_is_ok(map[i][j + 1]) == 1
